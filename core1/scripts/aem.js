...
function decorateIcon(span, prefix = '', alt = '') {
  ...
  img.src = `${window.hlx.codeBasePath}/core${prefix}/icons/${iconName}.svg`;
  img.alt = alt;
  img.loading = 'lazy';
  img.width = 16;
  img.height = 16;
...
async function loadBlock(block) {
  ...
  if (status !== 'loading' && status !== 'loaded') {
    ...
    try {
      const cssLoaded = loadCSS(`${window.hlx.codeBasePath}/core/blocks/${blockName}/${blockName}.css`);
      const decorationComplete = new Promise((resolve) => {
        (async () => {
          try {
            const mod = await import(
              `${window.hlx.codeBasePath}/core/blocks/${blockName}/${blockName}.js`
            );
            if (mod.default) {
